...
facedet.load_weights(os.path.join(HELPERS_DIR, "blazeface.pth"))
facedet.load_anchors(os.path.join(HELPERS_DIR, "anchors.npy"))

_ = facedet.train(False)

def load_cvit(cvit_weight, net, fp16):
    
    from .cvit import CViT as cvit # load cvit2
    if net == 'cvit':
        from .cvit_old import CViT as cvit

    model = cvit(image_size=224, patch_size=7, num_classes=2, channels=512,
             dim=1024, depth=6, heads=8, mlp_dim=2048)

    model.to(device)
    checkpoint = torch.load(cvit_weight, map_location=torch.device('cpu'))
