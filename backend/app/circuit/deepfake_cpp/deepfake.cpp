...
void ZKFaceVerifier_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void ZKFaceVerifier_2_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[3] = { 
Num2Bits_0_run,
LessThan_1_run,
ZKFaceVerifier_2_run };
...
ctx->componentMemory[coffset].templateName = "ZKFaceVerifier";
ctx->componentMemory[coffset].signalStart = soffset;
...
LessThan_1_create(mySignalStart+4,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[0] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[0];
...
ZKFaceVerifier_2_create(1,0,ctx,"main",0);
ZKFaceVerifier_2_run(0,ctx);
}
